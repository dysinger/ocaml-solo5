...
ocaml/Makefile.config: $(LIBS) $(TOOLCHAIN_FOR_BUILD) | ocaml
	...
	cd ocaml && \
	  ./configure \
		--target="$(MAKECONF_TARGET_ARCH)-none" \
...
$(OCAML_IS_BUILT): ocaml/Makefile.config | _build
	...
	cd ocaml && \
	  ...
	  $(MAKE) stdlibcomp OCAMLRUN=../runtime/ocamlrun OLDS="-o yacc/ocamlyacc -o lex/ocamllex" && \
	  $(MAKE) ocamlc OCAMLRUN=../runtime/ocamlrun OLDS="-o yacc/ocamlyacc -o lex/ocamllex" && \
